...
import { analyzePushUpForm, PushUpRepCounter, PushUpVariation } from '@/lib/pushUpAnalyzer';
...
const VARIATIONS: { id: PushUpVariation; label: string; emoji: string; description: string }[] = [
  { id: 'standard', label: 'ปกติ', emoji: '💪', description: 'Push-up พื้นฐาน' },
  { id: 'knee', label: 'บนเข่า', emoji: '🦵', description: 'ง่ายกว่า เหมาะสำหรับเริ่มต้น' },
  { id: 'incline', label: 'มือสูง', emoji: '🛋️', description: 'วางมือบนที่สูง ง่ายกว่า' },
  { id: 'close', label: 'มือชิด', emoji: '💎', description: 'Diamond เน้น Triceps' },
  { id: 'wide', label: 'มือกว้าง', emoji: '🦅', description: 'เน้นอก' },
];
...
export default function PushUpPage() {
  ...
  const [variation, setVariation] = useState<PushUpVariation>('standard');
  ...
  const handlePoseDetected = useCallback((landmarks: PoseLandmark[]) => {
    ...
    const formAnalysis = analyzePushUpForm(landmarks, smoothedElbow, elbowVelocity ?? undefined, variation);
    ...
    // Bad form feedback (only when in down phase for push-ups)
    if (formAnalysis.phase === 'down' && formAnalysis.score < 60) {
      // Find the worst issue from feedback
      const messages = formAnalysis.feedback.map(f => f.message);
      if (messages.some((m: string) => m.includes('สะโพก') && m.includes('สูง'))) {
        feedbackBadForm('hipsTooHigh');
      } else if (messages.some((m: string) => m.includes('สะโพก') && m.includes('ต่ำ'))) {
        feedbackBadForm('hipsTooLow');
      } else if (messages.some((m: string) => m.includes('หลัง'))) {
        feedbackBadForm('backNotStraight');
      } else if (messages.some((m: string) => m.includes('ข้อศอก'))) {
        feedbackBadForm('elbowsFlared');
      }
    }
            ...
            {recordedVideo ? (
              ...
              <>
                <div className="mb-3 sm:mb-4 bg-gray-900/50 rounded-lg p-4 sm:p-6 text-center">
                  ...
                  <p className="text-gray-400 text-sm">ไม่มีวิดีโอ (กล้องไม่ได้บันทึก)</p>
                </div>
